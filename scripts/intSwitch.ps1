If ("DevNATSwitch" -in (Get-VMSwitch | Select-Object -ExpandProperty Name) -eq $FALSE) {
    'Creating Internal-only switch named "DevNATSwitch" on Windows Hyper-V host...'
    New-VMSwitch -SwitchName "DevNATSwitch" -SwitchType Internal
}
else {
    '"DevNATSwitch" for static IP configuration already exists; skipping'
}
...
If ("172.16.0.1" -in (Get-NetIPAddress | Select-Object -ExpandProperty IPAddress) -eq $FALSE) {
    ...
    New-NetIPAddress -IPAddress 172.16.0.1 -PrefixLength 24 -InterfaceAlias "vEthernet (DevNATSwitch)"
}
else {
